...
extension xProgressHUD {
    ...
    func addEatBeansAnime()
    {
        ...
        // 路径动画
        let beanAnime = self.newAnimation(keyPath: "path", duration: duration * 4, autoreverses: false)
        beanAnime.fromValue = beanPath1.cgPath
        beanAnime.toValue = beanPath2.cgPath
        // 添加路径
        self.addAnimationLayer(frame: frame, fillColor: beanColor, anime: beanAnime)
        ...
        // 路径动画
        let mouthAnime = self.newAnimation(keyPath: "path", duration: duration, autoreverses: true)
        mouthAnime.fromValue = mouthPath1.cgPath
        mouthAnime.toValue = mouthPath2.cgPath
        // 添加路径
        self.addAnimationLayer(frame: frame, fillColor: mouthColor, anime: mouthAnime)
        ...
        // 路径动画
        let eyeAnime = self.newAnimation(keyPath: "path", duration: duration, autoreverses: true)
        eyeAnime.fromValue = eyePath1.cgPath
        eyeAnime.toValue = eyePath2.cgPath
        // 添加路径
        self.addAnimationLayer(frame: frame, fillColor: eyeColor, anime: eyeAnime)
    }
}
