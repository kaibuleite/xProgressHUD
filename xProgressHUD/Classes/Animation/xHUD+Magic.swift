...
extension xProgressHUD {
    ...
    func addMagicAnime()
    {
        ...
        // 路径动画
        let circle1Anime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
        circle1Anime.fromValue = 0
        circle1Anime.toValue = 1
        let circle1AnimeGroup = self.newAnimationGroup(animations: [circle1Anime], waitingDuration: waitingDuration)
        // 添加路径
        self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: circle1Path, anime: circle1AnimeGroup)
        ...
        // 路径动画
        let circle2Anime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
        circle2Anime.fromValue = 0
        circle2Anime.toValue = 1
        let circle2AnimeGroup = self.newAnimationGroup(animations: [circle2Anime], waitingDuration: waitingDuration)
        // 添加路径
        self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: circle2Path, anime: circle2AnimeGroup)
        ...
        // 类型1——带边框
        if self.config.animeType == .magic1 {
            // MARK: - 外部边框
            // 路径
            ...
            // 路径动画
            let hexagonAnime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
            hexagonAnime.fromValue = 0
            hexagonAnime.toValue = 1
            let hexagonAnimeGroup = self.newAnimationGroup(animations: [hexagonAnime], waitingDuration: waitingDuration)
            // 添加路径
            self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: hexagonPath, anime: hexagonAnimeGroup)
            ...
            // 路径动画
            let starAnime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
            starAnime.fromValue = 0
            starAnime.toValue = 1
            let starAnimeGroup = self.newAnimationGroup(animations: [starAnime], waitingDuration: waitingDuration)
            // 添加路径
            self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: starPath, anime: starAnimeGroup)
        }
        // 类型2——不带边框
        else {
            // MARK: - 正三角
            ...
            // 路径动画
            let upTriangAnime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
            upTriangAnime.fromValue = 0
            upTriangAnime.toValue = 1
            let upTriangAnimeGroup = self.newAnimationGroup(animations: [upTriangAnime], waitingDuration: waitingDuration)
            // 添加路径
            self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: upTrianglePath, anime: upTriangAnimeGroup)
            ...
            // 路径动画
            let downTriangAnime = self.newAnimation(keyPath: "strokeEnd", duration: duration)
            downTriangAnime.fromValue = 0
            downTriangAnime.toValue = 1
            let downTriangAnimeGroup = self.newAnimationGroup(animations: [downTriangAnime], waitingDuration: waitingDuration)
            // 添加路径
            self.addAnimationLayer(frame: frame, lineWidth: lineWidth, lineColor: lineColor, path: downTrianglePath, anime: downTriangAnimeGroup)
        }
    }
}
